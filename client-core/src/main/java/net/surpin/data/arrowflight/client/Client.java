...
import net.surpin.data.arrowflight.client.util.DurationFormatter;
    ...
    public RowSet fetch(Endpoint ep, Schema schema) {
        return retryWithBackoff(() -> {
            ...
            LOGGER.debug("node={} client=fetchCompleted batches={} rows={} elapsed={}",
                    NODE, batches, rows, DurationFormatter.elapsedNanos(startNanos));
            return rs;
        }, "fetch");
    }
                ...
                while (stream.next()) {
                    ...
                    if (batches == 1) {
                        LOGGER.debug("node={} client=ttfB batchRowCount={} elapsed={}",
                                NODE, root.getRowCount(), DurationFormatter.elapsedNanos(startNanos));
                    }
    ...
    public long execute(String stmt) {
        return retryWithBackoff(() -> {
            ...
            LOGGER.debug("node={} client=executeCompleted totalRows={} endpoints={} elapsed={}",
                    NODE, count, fi.getEndpoints().size(), DurationFormatter.elapsedNanos(startNanos));
            return count;
        }, "execute");
    }
