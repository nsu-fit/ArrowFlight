...
Инвентарь является неизменяемым startup snapshot. После изменения dataset нужен
координированный перезапуск всех Flight-нод; смешивание разных поколений
инвентаря не поддерживается.
