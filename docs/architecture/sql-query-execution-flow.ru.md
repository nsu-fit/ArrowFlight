...
Планирование endpoint и `DoGet` не хранят query payload в Hazelcast. Для
каждого endpoint Hazelcast хранит 10-минутный load lease, чтобы брошенный
ticket не оставлял нагрузку навсегда.
...
Дальше сервер читает список зарегистрированных Flight-нод из `serverRegistry`
и свежие `NodeLoadSnapshot` из Hazelcast. Для каждого файла планировщик
сравнивает зарезервированные байты, наблюдаемый throughput, занятые и ожидающие
слоты, CPU, память и локальность данных.

Для HDFS и другого общего хранилища любая живая нода может прочитать файл:
локальный блок уменьшает score, но перегруженная локальная нода может проиграть
свободной удалённой. Для локальной файловой системы owner файла остаётся
жёстким ограничением. Файлы распределяются от больших к меньшим, чтобы раньше
сбалансировать самые дорогие задачи.
...
файлов и эксклюзивно claim'ит endpoint lease, затем запрашивает permit у
`AdaptiveAdmissionController`. При отсутствии свободного execution slot запрос
ждёт в ограниченной FIFO-очереди.

Если локальная нода стала перегруженной, ещё не начавшийся endpoint может быть
атомарно переназначен другой ноде. Сервер возвращает replacement URI и
подписанный ticket в error metadata, а клиент повторяет только этот `DoGet`.
Первый Arrow batch ещё не был передан, поэтому данные не дублируются.
Переполнение или timeout без подходящей target-ноды возвращаются как обычный
`RESOURCE_EXHAUSTED`. После получения permit вызывается
...
Подписанный endpoint ticket самодостаточен. Load lease endpoint зависит от
Hazelcast, причем его истечение влияет на accounting, а не на декодирование
ticket.
