        ...
        try {
            ...
            Dataset<Row> df = spark.range(0, numRows, 1, numPartitions)
                    .select(
                            ...
                            functions.col("id").cast(INTEGER_TYPE).alias("id"),
                            ...
                            functions.col("id").mod(10).cast(INTEGER_TYPE).alias("int_col"),
                            ...
                            functions.concat(
                                    functions.lit("20"),
                                    functions.format_string("%02d", functions.floor(functions.rand(49).multiply(24)).plus(1).cast(INTEGER_TYPE)),
                                    functions.lit("-"),
                                    functions.format_string("%02d", functions.floor(functions.rand(50).multiply(12)).plus(1).cast(INTEGER_TYPE)),
                                    functions.lit("-"),
                                    functions.format_string("%02d", functions.floor(functions.rand(51).multiply(28)).plus(1).cast(INTEGER_TYPE))
                            ).alias("date_string_col"),
                            // string_col: случайная строка в формате "str_<num>"
                            functions.concat(
                                    functions.lit("str_"),
                                    functions.lpad(functions.rand(52).multiply(1000000).cast(INTEGER_TYPE).cast("string"), 6, "0")
                            ).alias("string_col"),
                            ...
                            functions.floor(functions.rand(54).multiply(25)).plus(2000).cast(INTEGER_TYPE).alias("year"),
                            // month: случайный месяц от 1 до 12
                            functions.floor(functions.rand(55).multiply(12)).plus(1).cast(INTEGER_TYPE).alias("month")
                    );
