...
final class EndpointTicketCodec {
    ...
    private static final int LEGACY_MAGIC = 0x41465431;
    private static final int MAGIC = 0x41465432;
    ...
    byte[] encode(HandleState state) {
        try {
            ...
            try (DataOutputStream output = new DataOutputStream(payloadBytes)) {
                ...
                writeString(output, state.reservationId());
    ...
    HandleState decode(byte[] encoded) {
        ...
        try (DataInputStream input =
            ...
            String reservationId = magic == MAGIC ? readString(input) : null;
            boolean loadTracked = input.readBoolean();
            if (input.available() != 0) {
                throw new IllegalArgumentException("Unexpected Flight endpoint ticket data");
            }
            return new HandleState(query, paths, serverUri, bytes,
                    reservationId, loadTracked);
        } catch (IOException e) {
    ...
    boolean isEncoded(byte[] encoded) {
        return encoded != null
                ...
                && (readInt(encoded) == MAGIC || readInt(encoded) == LEGACY_MAGIC);
    }
